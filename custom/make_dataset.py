...
def many_rain(dir_train, UPPER):
    ...
    with open(f'inputs/train_{UPPER}up.pickle', 'wb') as f:
        pickle.dump(train, f, protocol=4)

    with open(f'inputs/train_y_{UPPER}up.pickle', 'wb') as f:
        pickle.dump(train_y, f, protocol=4)
...
def augmentation(dir_train):
    ...
    for i in tqdm(os.listdir(dir_train)):
        ...
        if (npy[:, :, -1] >= 0.1).sum() > 100:
            # 2번째인자가 1이면 90도, 2이면 180도, 3번째 인자는 어떤 축을 잡고 돌릴것인지, 0번축, 1번축을 기준으로 돌릴것이다
            npy = np.rot90(npy, 3, (0, 1))
